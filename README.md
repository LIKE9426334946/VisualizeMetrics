...
CSV 上传后保存到自己的服务器，浏览器负责解析和绘图。Node.js 提供目录、文件和工作区存储接口，使用本地文件与 JSON，不需要数据库或登录系统。页面代码与依赖均由自己的服务器提供，不依赖外部 CDN。刷新或重启服务后会恢复上次的图表。

## 功能

- 点击上传或拖拽，一个或多个 CSV 同时导入；首次导入自己的文件时自动替换模拟示例。
- 创建和删除目录，在当前目录上传、查看和删除多个 CSV；支持跨目录勾选文件进行对比。浏览目录不会清空当前图表。
- 自动保存所选文件、模型名称、指标勾选、每条曲线的显示/极值标记、图表设置、缩放范围和原始表格状态。刷新后直接恢复，无需重新上传。
...
- 当前目录的模型汇总表：按指定指标的 Max / Min / Last 排序，自选字段及对应 Epoch，支持各指标独立统计或同一最佳 Epoch 的整行对比，下载完整高清 PNG / SVG。
...
1. 左侧“目录”点击 `+` 创建目录，点击目录名切换当前目录。首次启动提供“默认目录”，支持创建多个一级目录。
2. 点击“上传 CSV”或拖拽文件，文件会上传到当前目录；同名 CSV 保存为独立文件，不会覆盖已有文件。
3. 勾选服务器中已保存的文件即可绘图。可以切换目录继续勾选，组合多个目录中的实验进行比较；取消勾选仅隐藏曲线，文件仍保留。
4. CSV 旁的垃圾桶会删除服务器文件。目录旁的垃圾桶会删除目录和其中全部 CSV，页面会显示删除确认。被删除文件对应的图表记录也会清理。
5. 顶部显示“正在保存…”或“已保存到服务器”。工作区在操作后自动保存；如果网络失败，会显示可点击的重试提示。浏览器仅缓存尚未保存的配置以应对快速刷新，不缓存 CSV 内容。

工作区是个人服务器上的一份共享状态，换设备也可恢复；多个页面同时修改时，以服务器最后接受的保存为准。模拟示例有明确标记，不会作为 CSV 上传到目录。更新前只存在旧页面内存中的 CSV，需要在更新后上传一次。

默认数据位置：

```text
/opt/VisualizeMetrics/data/
├── index.json                        # 目录、文件索引及当前工作区配置
└── uploads/
    └── <目录 ID>/
        └── <文件 ID>.csv             # 保留上传 CSV 的原始字节
```

目录名、原始文件名记录在索引中；磁盘使用独立 ID 防止重名覆盖。备份时保存整个 `data/` 目录即可。此目录已加入 `.gitignore`，不会随源码提交到 GitHub；`git pull`、构建及安装脚本不会清空它。可通过 `DATA_DIR` 指定另一数据目录，服务配置默认为 `/opt/VisualizeMetrics/data`。

## 当前目录的模型汇总

图表下方的“模型汇总”自动读取当前目录中的全部 CSV，每份 CSV 占一行。未勾选绘图的文件也会参与汇总；其他目录的文件和模拟示例不会混入。模型名称与左侧已设置的名称一致，同名 CSV 仍是独立的模型记录。

- 默认优先使用 `val_iou` 的最大值降序排列，最高的模型在第一行；没有该字段时使用目录中可用的数值指标。可以切换排序指标、Max / Min / Last，以及升序或降序。选择最小验证损失排序时，使用 Min 和升序即可。
- 展开“显示字段与 Epoch”，自由选择需要显示的列、每个数值指标的 Max / Min / Last，以及是否显示该指标对应的 Epoch；也支持全选和取消全部。排序指标这一列跟随上方的“排序取值”，避免显示值与排名依据不一致。
- **各指标独立统计**：例如 Max `val_iou`、Max `val_f1`、Min `val_loss` 分别计算，每项极值可能来自不同 Epoch；勾选各自的 Epoch 列即可查看。单独选中的 `epoch` 或文本字段来自排序指标对应的原始行。
- **排序指标所在 Epoch 的整行**：例如以 Max `val_iou` 排序时，`epoch`、`val_loss`、`val_f1` 等所有选中字段均取该模型最佳 IoU 所在的同一原始行，不会替换成其他 Epoch 的极值。
- 所有统计基于完整 CSV，与曲线是否显示、图表缩放和原始数据搜索无关。Epoch 始终使用 CSV 的 `epoch`，无该列时使用数据行号；无效 Epoch 不参与统计。极值并列取最早 Epoch，重复 Epoch 保留原始行顺序，Last 为最后一个含有效指标值的 Epoch。
- 缺失指标、空值和无效数值显示 `—`；缺少排序指标有效值的模型始终放在末尾。排序使用完整数值，显示精度不会改变排名；模型之间数值并列时保留文件顺序。
- 排序、字段选择、取值方式及精度按目录分别自动保存，刷新或切换目录后恢复。
- “下载表格 PNG · 2×”输出两倍分辨率图片，“SVG”输出矢量图片。两者都包含目录标题、排序与取值说明、所有模型和所选字段，不会截断横向或纵向滚动区域外的数据。过大的表格可使用 SVG 完整导出。

若某个 CSV 读取失败，页面会提示并提供重试；读取完成前禁用导出，以免下载不完整的对比结果。
...
VisualizeMetrics/
├── backend/
│   ├── server.js                 # Node HTTP 服务；默认 127.0.0.1:3045
│   ├── api.js                    # 目录、CSV、工作区接口
│   └── storage.js                # 本地文件存储、串行写入与原子索引替换
├── frontend/
...
│       ├── summary-data.js       # 模型排名、独立极值和最佳行计算
│       ├── summary.js            # 当前目录汇总与字段配置
│       ├── summary-export.js     # 完整汇总表 SVG / 高清 PNG
│       ├── main.js               # 页面状态和交互
│       ├── server.js             # 上传、读取、自动保存及刷新恢复
│       ├── styles.css            # 响应式页面样式
...
本次更新包含 Nginx 的 50 MB 上传大小配置和 systemd 数据目录配置，请执行安装脚本同步这两份配置。原有的 `data/` 会保留。
...
另开终端运行前端，Vite 会将 `/api` 请求代理到 `127.0.0.1:3045`：
...
测试覆盖 CSV BOM/引号/换行、自定义/重复表头、行宽不一致、空值与非数值、无效 Epoch、极值并列、Last、不同模型 Epoch 长度、数值排序、CSV 原始字节保存、同名文件、并发写入、服务重启后的工作区恢复、延迟保存排序及文件/目录删除。汇总测试覆盖模型排名、独立极值与对应 Epoch、最佳行取值、缺失指标、重复 Epoch、列选择及显示精度。
